...
contract Drone is IDrone {
    ...
    function checkHigh (uint256 _droneId, uint256 _minAltitude, uint256 _maxAltitude) private view returns (bool droneValid) {
         
        droneValid = droneList[_droneId].minAltitude >= _minAltitude && droneList[_droneId].maxAltitude <= _maxAltitude;
        return droneValid;
    }
    
    function checkPesticide (uint256 _droneId, uint256 _pesticide) private view returns (bool droneValid) {
