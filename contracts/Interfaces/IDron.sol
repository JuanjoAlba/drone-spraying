...
interface IDron {
    ...
    function getDron(uint256 _dronId) external view returns (string name, uint256 minAltitude, uint256 maxAltitude, uint256[] pesticideList, uint256 cost);
    
    // Search dron by altitud and pesticide
    function searchDronBy(address _address, uint256 _minAltitude, uint256 _maxAltitude, uint256 _pesticide) external view returns (
            uint256 dronId,
            uint256 cost);
